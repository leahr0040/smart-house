...
const STATE_TYPE_NAME: Record<(typeof DEVICE_TYPES)[number], string> = {
  light: 'LightState',
  ac: 'AcState',
  heater: 'HeaterState',
  sensor: 'SensorState'
...
test('POST /rooms/:roomPublicId/devices creates a device owned by the caller', async (t) => {
  ...
  assert.ok(!('stateId' in body), 'response must not expose the state pointer')
})
...
test('device create inserts exactly one matching per-type state row with defaults (STATE-01)', async (t) => {
  ...
  for (const deviceType of DEVICE_TYPES) {
    ...
    assert.strictEqual(device?.stateType, STATE_TYPE_NAME[deviceType])
    assert.ok(device?.stateId, 'stateId must be set')
    ...
    assert.strictEqual((state as { id: bigint }).id, device?.stateId, 'device.stateId must point at the created state row')
...
test('PATCH /devices/:devicePublicId rejects a device_type change (D-06)', async (t) => {
  const app = await build(t)
  const { userA } = await createTwoTestUsers(app)
  const house = await seedHouse(userA.userId)
  const room = await seedRoom(userA.userId, house.id)
  ...
  const res = await app.inject({
    ...
    url: `/devices/${device.publicId}`,
    headers: authHeader(userA.token),
    payload: { deviceType: 'ac' }
  })

  assert.strictEqual(res.statusCode, 400)
})

test('DELETE /devices/:devicePublicId soft-deletes the leaf device (TEST-05)', async (t) => {
  const app = await build(t)
  const { userA } = await createTwoTestUsers(app)
  const house = await seedHouse(userA.userId)
  const room = await seedRoom(userA.userId, house.id)
