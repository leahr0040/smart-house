...
- **Consistency model** (single store):
  ...
  - *Ordering*: the state-row update is guarded — apply only if `(last_event_at, last_event_id) < (:recorded_at, :id)`, where `last_event_id` and `id` are the BigInt `events.id` (insertion-ordered tiebreaker). The deterministic uuidv5 `event_id` is an idempotency key, **not** an ordering key, so ordering never compares it. The event row is still recorded even when a stale report doesn't move current state.
